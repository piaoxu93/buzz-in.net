...
$key="<div class=\"pages\">"; 
if($page!=1){ 
$key.="<a class=\"page\" href=\"".$_SERVER['PHP_SELF']."?page=1\"><span>«</span>第一页</a> "; //第一页 
$key.="<a class=\"page\" href=\"".$_SERVER['PHP_SELF']."?page=".($page-1)."\"> 上一页</a>"; //上一页 
}else { 
$key.="<a class=\"page\"><span>«</span>第一页</a> ";//第一页 
$key.="<a class=\"page\"> 上一页</a>"; //上一页 
} 
if($pages>$page_len){ 
//如果当前页小于等于左偏移 
if($page<=$pageoffset){ 
...
if($page!=$pages){ 
$key.="<a class=\"page\" href=\"".$_SERVER['PHP_SELF']."?page=".($page+1)."\"> 下一页</a> ";//下一页 
$key.="<a class=\"page\" href=\"".$_SERVER['PHP_SELF']."?page={$pages}\"> 最后一页<span>»</span></a>"; //最后一页 
}else { 
$key.="<a class=\"page\"> 下一页</a> ";//下一页 
$key.="<a class=\"page\"> 最后一页<span>»</span></a>"; //最后一页 
}
$key.='</div>'; 
...
$Page_size=10; 
$comments = array();
$result2 = mysql_query("SELECT * FROM comment_china where ID=$ID ORDER BY pid desc");
...
$offset=$Page_size*($page-1); 
$comments = array();
$sql="select * from comment_china where ID=$ID order by pid desc limit $offset,$Page_size"; 
$result=mysql_query($sql,$link); 
...
$key="<div class=\"pages\">"; 
if($page!=1){ 
$key.="<a class=\"page\" href=\"".$_SERVER['PHP_SELF']."?ID=$ID&page=1\"><span>«</span>第一页</a> "; //第一页 
$key.="<a class=\"page\" href=\"".$_SERVER['PHP_SELF']."?ID=$ID&page=".($page-1)."\"> 上一页</a>"; //上一页 
}else { 
$key.="<a class=\"page\"><span>«</span>第一页</a> ";//第一页 
$key.="<a class=\"page\"> 上一页</a>"; //上一页 
} 
...
if($page!=$pages){ 
$key.="<a class=\"page\" href=\"".$_SERVER['PHP_SELF']."?ID=$ID&page=".($page+1)."\"> 下一页</a> ";//下一页 
$key.="<a class=\"page\" href=\"".$_SERVER['PHP_SELF']."?ID=$ID&page={$pages}\"> 最后一页<span>»</span></a>"; //最后一页 
}else { 
$key.="<a class=\"page\"> 下一页</a> ";//下一页 
$key.="<a class=\"page\"> 最后一页<span>»</span></a>"; //最后一页 
}
