...
mysql_query("SET NAMES 'UTF8'");
  ...
  //      return $con;
  //  }
       $myRss = new RSS("Buzz-in.net-Australia/Events","http://buzz-in.net/Australia/events.php","has an opinion about everything");
    //$myRss->RSS();
    ...
    while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
                ...
                $myRss->AddItem($row1->question,"http://buzz-in.net/Australia/question.php?ID=".$row['ID'],'events',"<div style='margin-bottom:3px;'>$t</div><img width='140' height='87'  style='clear:both;' src=\"display_picture.php?ID=$row1->ID\"/><div>".$row1->brief_text."</div>",$row['time']);
            }
    $myRss->BuildRSS();
